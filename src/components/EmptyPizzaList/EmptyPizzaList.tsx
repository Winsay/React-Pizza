...
import style from './EmptyPizzaList.module.scss';
import { useAppDispatch } from '../../Hooks/hooks';
import { ActionCreatorWithPayload } from '@reduxjs/toolkit';

interface IEmptyPizzaListProps {
  setCurrentPage: ActionCreatorWithPayload<number, 'main/setCurrentPage'>;
  setSearchValue: ActionCreatorWithPayload<string, 'main/setSearchValue'>;
}

const EmptyPizzaList: React.FC<IEmptyPizzaListProps> = ({ setSearchValue, setCurrentPage }) => {
  ...
  return (
    <div className={style.emptyPizzas}>
      <svg
        className={style.sadIco}
        ...
        <defs>
          ...
          <radialGradient
            ...
            id="radialGradient-2"
            r="49.9682142%">
            <stop offset="0%" stopColor="#D27C12" stopOpacity="0.186216128" />
            <stop offset="85.6313345%" stopColor="#D27C12" stopOpacity="0" />
            <stop offset="100%" stopColor="#D27C12" stopOpacity="0.324546547" />
          </radialGradient>
          <linearGradient
            id="linearGradient-3"
            x1="54.0185272%"
      ...
      <h2 className={style.description}>
        К сожалению <br /> по вашему запросу ничего не найдено
      </h2>
      <button onClick={clearSearch} className={style.clearBtn}>
        Oчистить поиск
      </button>
    </div>
  );
};
export default EmptyPizzaList;
